...
class MIDS(nn.Module):
    def __init__(self, dim=768, **kwargs):
        ...
        # PAAS: default to the bundled encoders (was relative 'models/...'); $PAAS_T5/$PAAS_CLIP override.
        import os as _os
        _t5 = _os.environ.get("PAAS_T5", "/datasets/work/vLLM/temp/PAAS_ensemble_v2/base_models/t5-base")
        _clip = _os.environ.get("PAAS_CLIP", "/datasets/work/vLLM/temp/PAAS_ensemble_v2/base_models/clip-vit-large-patch14-336")
        self.text_model_path = kwargs.pop('text_model_path', _t5)
        self.image_model_path = kwargs.pop('image_model_path', _clip)
    ...
    def get_cls_loss(self, logits, labels, ignore_index=-1):
        weights = torch.tensor([1.0, 1.0, 1.0, 1.0]).to(logits.device)
        # weights = torch.tensor([1.0, 1.0, 7.0, 1.0]).to(logits.device)
        # weights = torch.tensor([1.0, 2.0, 2.0, 1.0]).to(logits.device)
        # weights = torch.tensor([1.0, 2.0, 1.0, 1.0]).to(logits.device) #zzzzzz
        loss_func = nn.CrossEntropyLoss(ignore_index=ignore_index, weight=weights)
