...
Examples:
  python3.12 test_video_image_batch.py --config config/experiments/weighted_ffaa.json \
      --input-dir /datasets/work/vLLM/data/axonlabs_data_1 --out-dir runs/test --devices all
  python3.12 test_video_image_batch.py --input-dir /data --device 0 --frame-stride 10 --flush-size 128
"""
import argparse
...
def gpu_worker(device_id, media_paths, args_dict, n_devices, result_q):
    ...
    if args.fusion:
        ...
        cfg.ffaa.enabled = args.fusion != "ensemble_only"
        cfg.ensemble9.enabled = args.fusion != "ffaa_only"
    if args.threshold is not None:
        cfg.decision.threshold = args.threshold
    cfg.validate()

    print(f"[GPU {device_id}] loading pipeline [{cfg.fusion.method}] "
    ...
    print(f"[GPU {device_id}] ready (ensemble={has_ens} ffaa={has_ffaa}) "
          f"threshold={cfg.decision.threshold}", flush=True)
...
def main():
    ...
    ap.add_argument("--config", default=os.path.join("config", "experiments", "weighted_ffaa.json"))
    ...
    ap.add_argument("--fusion", default=None, help="override fusion method")
    ...
    if has_ens and has_ffaa:
        print(f"offline frontier: python3.12 scripts/combine_eval.py "
